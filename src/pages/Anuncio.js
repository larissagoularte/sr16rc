...
const Anuncio = () => {
  ...
  const handleMediaClick = (index) => {
    setSelectedMedia(mediaFiles[index]);
          ...
          <div className="flex flex-col gap-1">
            <div className="MAIN-IMAGEM relative main-img h-[500px]">
                {selectedMedia.endsWith('.mp4') ? (
                    ...
                    <img src={selectedMedia} className="w-full h-full object-contain" alt="Imagens do anúncio" />
                )}
              ...
              <div ref={galleryRef} className="TAB-GALERIA-IMGS flex overflow-hidden">
                {mediaFiles.map((media, index) => (
                  <div key={index} className={`h-20 w-20 flex-shrink-0 overflow-hidden mx-1 ${currentIndex === index ? 'selected' : ''}`}>
                    {media.endsWith('.mp4') ? (
                        <video className="h-full w-full object-cover cursor-pointer" onClick={() => handleMediaClick(index)}>
                        <source src={media} type="video/mp4" />
                        Your browser does not support the video tag.
                      </video>
                    ) : (
                        <img src={media} alt={`Imagens do imóvel`} className="h-full w-full object-cover cursor-pointer" onClick={() => handleMediaClick(index)} />
                    )}
                  </div>
                ))}
              </div>
